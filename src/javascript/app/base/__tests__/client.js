    ...
    describe('.getUpgradeInfo()', () => {
        ...
        it('returns as expected for accounts that can upgrade to real', () => {
            ['svg', 'malta', 'iom'].forEach((lc) => {
                ...
                expect(upgrade_info.getUpgradeLink()).to.eq('new_account/realws');
                expect(upgrade_info.isCurrentPathAllowed()).to.eq(false);
            });
        });
        it('returns as expected for accounts that can upgrade to financial', () => {
            ...
            const upgrade_info = Client.getUpgradeInfo();
            expect(upgrade_info.getUpgradeLink()).to.eq('new_account/maltainvestws');
            expect(upgrade_info.isCurrentPathAllowed()).to.eq(false);
        });
        it('returns as expected for multi account opening', () => {
            ...
            const upgrade_info = Client.getUpgradeInfo();
            expect(upgrade_info.upgrade_links).to.deep.eq({});
            expect(upgrade_info.isCurrentPathAllowed()).to.eq(false);
        });
    });
